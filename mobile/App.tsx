...
function getWeatherIcon(condition: string): string {
  ...
  if (condition.includes("くもり")) {
    return "☁️";
  }
  if (condition.includes("晴") || condition.includes("快晴")) {
    return "☀️";
  }
  return "🌤️";
}
...
function splitMarkdownLinks(line: string): PlanSegment[] {
  ...
  return (
    <View style={styles.weatherBox}>
      ...
      <Text style={styles.body}>
        現在: {currentIcon} {weather.current.condition} / {weather.current.temperature ?? "-"}度 / 風速{" "}
        {weather.current.wind_speed ?? "-"} m/s
      </Text>
      {weather.daily.map((day) => (
        <Text key={day.date} style={styles.weatherLine}>
          {day.is_trip_day ? "旅行日 " : ""}
          {day.date}: {getWeatherIcon(day.condition)} {day.condition} ({day.temperature_min ?? "-"}度〜
          {day.temperature_max ?? "-"}度)
        </Text>
      ))}
    </View>
  );
  ...
  return (
    <View style={styles.planText}>
      {lines.map((line, lineIndex) => {
        const segments = splitMarkdownLinks(line);
        const isBlank = line.trim().length === 0;

        return (
          <Text key={`${lineIndex}-${line}`} style={[styles.planLine, isBlank && styles.planBlankLine]}>
